...
y_proba = probs = np.exp(dec_probs) / np.sum(np.exp(dec_probs))
y_proba
...
#There is no predict_proba method for Ridge Classifier, since Ridge Classifier
#outputs values from -1 to +1 and can be viewd by running dec_probs = grid_result.decision_function(X_test)

#Get outputs of the decision function
dec_probs = grid_result.decision_function(X_test)

#convert decision function outputs to 0,1 range using softmax transformation
y_proba = probs = np.exp(dec_probs) / np.sum(np.exp(dec_probs))

#Rest of the code remains the same as for the other models
